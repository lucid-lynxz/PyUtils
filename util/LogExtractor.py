...
class LogExtractor:
    ...
    def update_zip_path(self, zip_path: str, **kwargs) -> Self:
        ...
        更新zip包路径, 并按需下载到本地
        @param zip_path: zip包本地路径或者可下载的url路径, 比如: http://xxx.aliyuncs.com/logs/测试日志_1775183375996.zip?OSSAccessKeyId=xx&Expires=xxx&Signature=xxx
        @param kwargs: 其他参数, 主要用于下载, 如: auth=('账号', '密码'), timeout=10
        """
        self.zip_path = zip_path
        if self.zip_path.startswith('http'):
            ...
            if CommonUtil.isNoneOrBlank(local_zip_path):
                raise Exception(f'下载文件失败: {self.zip_path}')
    ...
    def batch_filter(
        ...
        Args:
            ...
            result_combine_flag: 日志过滤后, 每种信息都是个列表, 将列表拼接成字符串时, 使用的分隔符, 默认时逗号
        ...
        for idx, row in df_to_download.iterrows():
            ...
            # 下载文件
            self.update_zip_path(log_url)
            local_path = self.zip_path

            if local_path:
                # 更新原DataFrame中的 local_log
                # 找到原DataFrame中对应的行索引
                original_idx = df.index[mask_need_download][idx]
                ...
                for key in filter_result_dict:
                    df.at[original_idx, key] = result_combine_flag.join(filter_result_dict[key])
