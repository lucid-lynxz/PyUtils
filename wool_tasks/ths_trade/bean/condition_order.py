        ...
        if delta >= expected_delta:
            ...
            if success and not CommonUtil.isNoneOrBlank(self.config_path) and self.row_number >= 0:
                ...
                if self.row_number < len(lines):
                    ...
                    write_success = FileUtil.write2File(self.config_path, '\n'.join(lines))
                    CommonUtil.printLog(f'将条件单配置文件中的对应项置为无效,写入结果:{write_success},行号:{self.row_number},内容:{self.row_str},文件路径:{self.config_path}')
